(() => {
  ...
  window.CMEvent={events,get(){const v=get.call(localStorage,EVENT_KEY);return events.includes(v)?v:'C108'},set(v){if(events.includes(v))set.call(localStorage,EVENT_KEY,v)},switch(v){if(events.includes(v)){this.set(v);location.reload()}}};
  ...
  const clean=()=>document.querySelectorAll('#eventTabs [data-event]').forEach(b=>{if(!events.includes(b.dataset.event))b.remove()});
