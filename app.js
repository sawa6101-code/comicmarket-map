...
function renderMarkers(){
 $('markers').innerHTML=state.circles.filter(c=>c.day===state.day&&c.map===state.map).map(c=>`<button class="marker ${c.status==='COMPLETED'?'completed':c.status==='SOLD_OUT'?'sold':c.status==='PLANNED'?'planned':''}" data-marker="${c.id}" title="${esc(c.name)}" style="left:${c.x*100}%;top:${c.y*100}%" aria-label="${esc(c.name)}"></button>`).join('');
}
